...
| `direction` | 空、1、3 为水平文字；2、4 为垂直文字 |
| `arrowdirection` | 逗号分隔多选：1 右、2 左、3 下、4 上 |

旧 `arrowdirection=5` 会读取为 `1,2`，旧值 `6` 会读取为 `3,4`；新页面不再生成旧双向枚举。
...
- 箭头方向使用右、左、下、上四个复选框，可同时选择多个方向。
- 相反方向显示为两条单向箭头，不使用双向箭头枚举。
- 不提供箭头旋转按钮。
- 文字可选择水平或垂直排列，`direction` 留空时默认水平。
